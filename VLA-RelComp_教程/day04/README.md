...
行为克隆属于监督式模仿学习：数据是 observation、instruction、action 序列；标签是示范 action，不是“模型自己探索得到奖励”。SFT 在这里指用有监督数据微调已有模型。用一条流程建立整体认识即可：

```text
采集示教 episode
  → 回放审计：图像、state、action、时序与任务是否对齐
  → 训练：从 observation + instruction 预测示范 action
  → open-loop 检查：在固定数据上比较预测与示范，不改变后续 observation
  → closed-loop 评测：把动作送入环境，下一帧由真实执行结果产生，最终由官方 goal 判 success
```

这五步解释了为什么 loss 下降仍可能闭环失败：训练和 open-loop 主要看到专家访问过的状态，closed-loop 中一次小误差会改变下一帧 observation，并把策略带入陌生状态。流程借鉴社区教程的组织方式，但事实字段、执行入口和 success 均以冻结 VLA-Arena 论文、代码与协议为准；它不是另一套必跑环境，也不新增采集或训练任务。
...
正常 6–8 小时。新增流程图替代原先抽象记忆 distribution shift，不增加阅读或实操时长。最低完成线：解释 BC 与闭环误差、运行验证器、列 Gate 1 字段。标准完成线：独立区分环境/模型/评测失败并完成 Gate 草稿。选做：用三步二维轨迹演示单步误差如何改变后续输入。
