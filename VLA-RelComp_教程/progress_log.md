...
- H2.5.1 事后复核：正式 tag-only clone 中 isolated validator 仍硬编码开发分支 ref，因此 isolated fresh 验收为 FAIL；既有 tag 保留不改，不再作为当前入口。
- H2.5.2：全部 release fixture 改为从 `RELEASE_TAG` 克隆，新增无本地开发分支的 tag-only source 完整回归；当前发布锁升级为 lightweight `vla-relcomp-h2.5.2`。
- 回归：H1 原回归、H2 31 文件/23 组离线检查、67 个教程+根入口/历史交接文档/代码路径/凭据扫描，以及 23 项 tag-only isolated fresh-checkout/本地 Git 回归通过。全部是 Mac/静态/dry-run/synthetic，未运行 GPU、模型、仿真或网络克隆。
