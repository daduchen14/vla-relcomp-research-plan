...
| C2 | random/base 无头 smoke | `h2_pilot.py --model random`，1 trial/task | 5/5 episodes 均以 success/timeout 结束；无渲染/环境异常；有 JSON+日志+至少一视频 | `environment` / `render` / `evaluation_io` | 10–30 min / <2 GB / 0.2–2 GB | 任一 BDDL/init 不能加载；EGL 排错 45 min 仍失败 | C2 config、stdout/stderr、result JSON、video、registry |
...
| C4 | SmolVLA 小 pilot | `h2_pilot.py --model smolvla`；L0 先 5 tasks×5 trials=25，通过后 L1/L2 同协议 | registry 唯一键完整；工程异常率可接受；L0 `k/25+CI`、中位墙钟、峰值显存可算 | `infrastructure` / `model` / `evaluation` / `data_quality` | L0 1–3 h；75 episodes 3–6 h / 待测 / 2–20 GB | L0 完成前不看 L1/L2；若 L0<8/25 或大面积模型失败，进 C5；不训练 | task-level registry、Gate 2 草表、三级 configs/logs/results |
...
| C7 | 最小 pair/oracle pilot | 预登记 2–3 pair；至少 2 seeds；none+一种 oracle 逐步 | goal/instruction 同步；seed/init 配对；可达；恢复与damage同报；Gate 3 只留一个合法结论 | `pair_invalid` / `unreachable` / `leakage` / `oracle_damage` / `not_reproducible` | 2–4 h / 沿用主模型 / 2–10 GB | pair 不成立、状态不可观测、结果不可重复则依 Gate 3/D1 停；不设计 D15 修复 | pair manifest、oracle spec、配对 registry、原始计数/CI、Gate 3 |
...
python3 "$H2_TUTORIAL/scripts/h2_pair_oracle_audit.py" \
  ...
  --registry "$H2_RUN/registry/episode_registry.csv" --require-ready \
  --output "$H2_RUN/results/c7_pair_oracle_audit.json"
```

第一条在运行前做结构/goal/可达/泄漏字段闸门；第二条只计 matched recovery/damage，不自动下 Gate 3 结论。
